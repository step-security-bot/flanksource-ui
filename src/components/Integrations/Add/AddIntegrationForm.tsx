import LogBackendsForm from "../../LogBackends/LogBackendsForm";
import TopologyResourceForm from "../../Topology/Settings/StepsForms/TopologyResourceForm";
import { CreateIntegrationOption } from "./steps/AddIntegrationOptionsList";
import CatalogFormOption from "./steps/CatalogFormOption";
import TemplateIntegrationOption from "./steps/TemplateIntegrationOption";

type Props = {
  onSuccess: () => void;
  selectedOption: CreateIntegrationOption;
  onBack: () => void;
};

export default function AddIntegrationForm({
  onSuccess,
  selectedOption,
  onBack
}: Props) {
  switch (selectedOption) {
    case "AWS":
    case "Azure":
    case "Prometheus":
    case "Flux":
    case "Kubernetes":
    case "Mission Control":
      return (
        <TemplateIntegrationOption
          selectedOption={selectedOption}
          onBack={onBack}
          onSuccess={onSuccess}
        />
      );
    case "Custom Topology":
      return (
        <TopologyResourceForm
          selectedOption={"Custom"}
          onBack={onBack}
          isModal
          onSuccess={onSuccess}
        />
      );
    case "Log Backends":
      return <LogBackendsForm onUpdated={onSuccess} />;
    case "Catalog Scraper":
      return <CatalogFormOption onSuccess={onSuccess} onBack={onBack} />;
    default:
      return null;
  }
}
